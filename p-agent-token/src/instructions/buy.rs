...
impl<'a> TryFrom<&'a mut [AccountView]> for BuyAccounts<'a> {
    ...
    fn try_from(accounts: &'a mut [AccountView]) -> Result<Self, Self::Error> {
        let [buyer, curve, _vault, buyer_token_account, _mint, token_program] = accounts else {
            return Err(ProgramError::NotEnoughAccountKeys);
        };
        require_signer(buyer)?;
        require_writable(curve)?;
        require_writable(buyer_token_account)?;
...
impl TryFrom<&[u8]> for BuyData {
    ...
    fn try_from(data: &[u8]) -> Result<Self, Self::Error> {
        ...
        let lamports_in = u64::from_le_bytes(
            data.try_into()
...
impl<'a> TryFrom<(&'a [u8], &'a mut [AccountView])> for Buy<'a> {
    ...
    fn try_from((data, accounts): (&'a [u8], &'a mut [AccountView])) -> Result<Self, Self::Error> {
        Ok(Self {
            accounts: BuyAccounts::try_from(accounts)?,
            data: BuyData::try_from(data)?,
        })
    }
}
